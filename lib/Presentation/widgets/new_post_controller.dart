...
class NewPostController {
  NewPostController._internal();
  static final _instance = NewPostController._internal();
  factory NewPostController() => _instance;

  List<NewPost>? _allPosts;

  // get addPosts => null;

  set addPost(NewPost newPost) {
    _allPosts ??= [];
    _allPosts!.insert(0, newPost);
  }

  set addPosts(List<NewPost> newPosts) {
    _allPosts ??= [];
    _allPosts!.addAll(newPosts);
  }

  List<NewPost>? get allPosts {
    return _allPosts;
  }
  void dispose() {
    _allPosts = null;
